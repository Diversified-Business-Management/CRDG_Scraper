...
async function fetchOne(target) {
  try {
    ...
    if (res.status >= 400) { console.warn(`[skip] ${target.slug}/${target.kind} HTTP ${res.status}`); return; }
    const fname = `${target.slug}-${target.kind}.html`;
    await writeFile(join(FIXTURE_DIR, fname), html, 'utf8');
    console.log(`[ok ] ${fname} (${(html.length / 1024).toFixed(1)} KB)`);
  } catch (err) { console.warn(`[err] ${target.slug}/${target.kind} ${err?.message || err}`); }
}

for (const t of TARGETS) { await fetchOne(t); await sleep(1100); }
